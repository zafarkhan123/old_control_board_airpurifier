...
void consolelogger_log_with_GetLastError(const char* file, const char* func, int line, const char* format, ...)
{
    ...
    */
    ...
    systemMessage = printf_alloc("Error: File:%s Func:%s Line:%d %s", file, func, line, lastErrorAsString);

    if (systemMessage == NULL)
    {
        systemMessage = "";
        (void)printf("Error: [FAILED] File : %s Func : %s Line : %d %s", file, func, line, lastErrorAsString);
